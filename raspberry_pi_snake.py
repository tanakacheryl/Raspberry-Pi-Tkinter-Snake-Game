...
# BUTTON SETUP
up_button = Button(26, pull_up=False)
down_button = Button(12, pull_up=False)
left_button = Button(25, pull_up=False)
right_button = Button(18, pull_up=False)
# Add a new button for starting a new game
start_button = Button(24, pull_up=False)
...
def up_pressed():
    ...
    if (snake_moved_in_this_frame == False):
        ...
        if (snake_move_dir[1] != 1):
            snake_move_dir  = [0, -1]
    print('UP Button pressed fr')
...
def left_pressed():
    ...
    if (snake_moved_in_this_frame == False):
        snake_moved_in_this_frame = True
        if (snake_move_dir[0] != 1):
            snake_move_dir  = [-1, 0]
    print('Left Button pressed fr')
...
def right_pressed():
    ...
    if (snake_moved_in_this_frame == False):
        ...
        if (snake_move_dir[0] != -1):
            snake_move_dir  = [1, 0]
        print('Right Button pressed fr')
right_button.when_pressed = right_pressed
def start_new_game():
    global snake_coords, snake_tail, snake_move_dir, apple_coords
    snake_coords = [game_dimensions[0] // 2, game_dimensions[1] // 2]
    snake_tail = []
    snake_move_dir = [1, 0]
    apple_coords = generateAppleCoords()
start_button.when_pressed = start_new_game
